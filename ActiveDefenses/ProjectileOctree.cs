...
namespace ActiveDefenses
{
    public class OctreeProj
    {
        ...
        {
            //Debug.Log("ActiveDefenses: OctreeProj(NavigateOctree) - CALLED");
            UpdatePos();
            ...
            for (int step = 0; step < count;)
            {
                ...
                if (!(bool)proj?.rbody)
                {
                    DebugActDef.Log("ActiveDefenses: OctreeProj(NavigateOctree) - error - RBODY is NULL");
                    Remove(proj);
                    found.RemoveAt(step);
                    count--;
                    continue;
                ...
                {
                    //Debug.Log("ActiveDefenses: OctreeProj(NavigateOctree) - Shooter null");
                    Remove(proj);
                    found.RemoveAt(step);
                    count--;
                    continue;
            ...
            {
                //Debug.Log("ActiveDefenses: OctreeProj(NavigateOctree) - EMPTY");
                return false;
            }
        ...
        {
            //Debug.Log("ActiveDefenses: OctreeProj(Remove) - CALLED");
        ...
        {
            //Debug.Log("ActiveDefenses: OctreeProj(Add) - CALLED");
        ...
        public class OctreeBranch
        {
            ...
            internal bool NavigateOctant(Vector3 pos, float range, Vector3 posMin, Vector3 posMax, int minCount, out List<Projectile> found)
            {
                ...
                if (!hasBranches)
                {
                    DebugActDef.Log("ActiveDefenses: OctreeBranch(NavigateOctant) - Trunk HAS NO BRANCHES OR PROJECTILES!!!");
                    return false;
                }
            ...
            private int NaviOctant(Vector3 pos, byte depthLevel, float range, Vector3 posMin, Vector3 posMax, int minCount, out List<Projectile> found, ref bool Done)
            {
                ...
                if (!hasBranches)
                {
                    DebugActDef.Log("ActiveDefenses: OctreeBranch(NaviOctant) - Illegal call");
                    return 0;
                }
                if (NotWithinCubeInv(posMin, posMax))
            ...
            {   // CALLED FROM WHERE IT IS
                if (!hasProjectiles)
                {
                    //Debug.Log("ActiveDefenses: OctreeBranch(RemoveProj) - Illegal call " + StackTraceUtility.ExtractStackTrace());
                    return false;
                }
                ...
                {
                    DebugActDef.Log("ActiveDefenses: OctreeBranch(RemoveProj) - Remove request failed " + StackTraceUtility.ExtractStackTrace());
                }
                return true;
            }
            public void CheckRemove()
            {
                ...
                if (hasBranches)
                {
                    ...
                    {
                        //Debug.Log("ActiveDefenses: OctreeBranch(CheckRemove) - Illegal call");
                        return;
                    }
                }
                ...
                //Debug.Log("ActiveDefenses: OctreeBranch(CheckRemove) - The host branch");
            }
            private bool TryRetractBranches()
            {
                try
                {
                    //Debug.Log("ActiveDefenses: OctreeProj(ExtendBranches) - CALLED");
                    int totCount = 0;
            ...
            public OctreeBranch AddProjectile(Projectile proj)
            {
                if (!hasBranches)
                {
                    //Debug.Log("ActiveDefenses: OctreeBranch(AddProjectile) - CALLED oct " + listDir + " lv " + scale);
                    if (!hasProjectiles)
                    ...
                    {
                        DebugActDef.Log("ActiveDefenses: OctreeProj(ExtendBranches)! - error " + e);
                    }
                    return this;
                }
                else
                {
                    //Debug.Log("ActiveDefenses: OctreeBranch(AddProjectile) - CALLED [Extend] oct " + listDir + " lv " + scale);
            ...
            private OctreeBranch ExtendBranch(byte index, Projectile toAdd)
            {
                if (branches[index] != null)
                {
                    DebugActDef.Log("ActiveDefenses: OctreeBranch(ExtendBranch) - Called when branch already exists");
                }
